This is an exercise on Test Driven Development for embedded systems. The cpputest framework's MockIO from [cpputest-starter-project](https://github.com/jwgrenning/cpputest-starter-project) is used to implement the _Erase Suspend & Resume_ flowchart on pg-44 in Appendix C of [the device specification](docs/STMicroelectronics/m28w160ect.pdf) mentioned in **Put the Knowledge to Work** section at the end of ***chapter 10*** of the book.<br />
...
4.) ```cd ex-erase-suspend-resume-flowchart/ex-unit-tests```
...
```sudo docker build -t ex-erase:latest ./```. <br /> 

Run using the docker engine <br />
```sudo docker run --rm ex-erase:latest```